 ...
 */
/*
import { TravelToOptions, TravelData, PathfinderReturn, TravelState } from "declarations/interfaces";
import { HasPos, Coord } from "declarations/types"; */
	...
	public static travelTo(creep: Creep, destination: HasPos | RoomPosition, options: TravelToOptions = {}): number {
		...
		if (!this.samePos(state.destination, destination)) {
			if (options.movingTarget && state.destination.isNearTo(destination)) {
				(travelData.path as string) += state.destination.getDirectionTo(destination);
